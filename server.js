...
const upload = multer({
  ...
  fileFilter: function (req, file, cb) {
    var allowed = ['audio/webm', 'audio/ogg', 'audio/wav', 'audio/x-wav', 'audio/mp4', 'audio/m4a', 'audio/mpeg'];
...
async function forwardToN8n(fileBuffer, filename, mimetype, chatId) {
  ...
  form.append('audio', fileBuffer, { filename: filename || 'segment.webm', contentType: mimetype || 'audio/webm' });
...
async function processWithGemini(fileBuffer, mimetype, chatId) {
  ...
  const body = {
    contents: [{
      role: 'user',
      parts: [
        { text: 'أنت المساعد الصوتي "إلفير". استمع للمقطع الصوتي التالي من المستخدم (معرّف المحادثة: ' + (chatId || 'unknown') + ') وردّ بإيجاز ووضوح باللغة العربية.' },
        { inline_data: { mime_type: mimetype || 'audio/webm', data: base64Audio } }
      ]
    }]
  };

  const response = await axios.post(url, body, { headers: { 'Content-Type': 'application/json' }, timeout: 25000 });
...
app.get('/health', function (req, res) {
  res.status(200).json({ status: 'ok', n8n_configured: Boolean(N8N_WEBHOOK_URL), gemini_configured: Boolean(GEMINI_API_KEY), websocket: 'ready' });
});

app.post('/api/audio', upload.single('audio'), async function (req, res) {
  try {
    if (!req.file) return res.status(200).json({ status: 'ignored', reason: 'no_audio_file' });
    ...
    if (N8N_WEBHOOK_URL) {
      try {
        ...
        source = 'n8n';
        if (n8nData) {
          if (typeof n8nData === 'string') replyText = n8nData;
          else if (typeof n8nData.reply_text === 'string') replyText = n8nData.reply_text;
          else if (typeof n8nData.reply === 'string') replyText = n8nData.reply;
        }
      } catch (n8nErr) { console.error('[audio] n8n forward failed:', n8nErr.message); }
    }
    ...
    if (!replyText && GEMINI_API_KEY) {
      ...
      } catch (geminiErr) { console.error('[audio] Gemini processing failed:', geminiErr.message); }
    }

    broadcastToChat(chatId, { type: 'audio_reply', chat_id: chatId, source: source, reply_text: replyText });

    return res.status(200).json({ status: 'ok', source: source, chat_id: chatId, reply_text: replyText });
  } catch (err) {
...
app.use(function (err, req, res, next) {
  if (err) return res.status(200).json({ status: 'error', message: err.message });
  next();
});
